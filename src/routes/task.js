...
// we can modify name, description and isComplete if the task has no SubTask
router.patch('/tasks/:id', async(req, res)=>{
    const updates = Object.keys(req.body)
    const allowedUpdates = ['name', 'description', 'isComplete']
    const isValid = updates.every(update=>allowedUpdates.includes(update))
    if(!isValid){
        return res.status(400).send("Query keys are not allowed. Allowed = 'name', 'description & 'isComplete")
    }

    try{
        if(updates.includes('isComplete')){
            console.log("is complete present")
            subs = await Subtask.find({task:req.params.id})
            if(subs.length > 0){
                return res.status(400).send("The task has subtask so you can't modifie isComplete")
            }
        }
